...
def unit_selectivity_analysis(datadir):
    ...
    for fnum, fname in enumerate(datafiles):
        ...
        for u in range(n_units):
            ...
            f_cue_ttest.at[u, 'type_p'] = pg.ttest(state_frs[type_labels == 1], state_frs[type_labels == 0])[
                'p-val'].values
            f_cue_ttest.at[u, 'mix_p'] = pg.ttest(state_frs[arbitrary_labels == 1], state_frs[arbitrary_labels == 0])[
                'p-val'].values
            f_cue_ttest.at[u, 'shuffle_p'] = pg.ttest(state_frs[shuffled_labels == 1], state_frs[shuffled_labels == 0])[
                'p-val'].values
            f_cue_ttest.at[u, 'state_d'] = pg.ttest(state_frs[state_labels == 1], state_frs[state_labels == 0])[
                'cohen-d'].values
            f_cue_ttest.at[u, 'type_d'] = pg.ttest(state_frs[type_labels == 1], state_frs[type_labels == 0])[
                'cohen-d'].values
            f_cue_ttest.at[u, 'mix_d'] = pg.ttest(state_frs[arbitrary_labels == 1], state_frs[arbitrary_labels == 0])[
                'cohen-d'].values
            f_cue_ttest.at[u, 'shuffle_d'] = pg.ttest(state_frs[shuffled_labels == 1], state_frs[shuffled_labels == 0])[
                'cohen-d'].values
...
def plot_cue_pop_summary(cue_ttests):
    ...
    plt.ylim([0, 19])
    plt.savefig('cue_pop_summary.svg', transparent=True)
    ...
    df['brain_area'] = np.concatenate((np.ones(shape=(np.sum(~ofc_ix),)), np.ones(shape=(np.sum(~ofc_ix),)),
                                       np.zeros(shape=(np.sum(ofc_ix),)), np.zeros(shape=(np.sum(ofc_ix),))),
                                      axis=0).astype(int)

    df['grouping'] = np.concatenate((np.ones(shape=(np.sum(~ofc_ix),)), np.zeros(shape=(np.sum(~ofc_ix),)),
                                     np.ones(shape=(np.sum(ofc_ix),)), np.zeros(shape=(np.sum(ofc_ix),))),
                                    axis=0).astype(int)
...
def check_val_by_state(choice_glms):
    ...
    plt.scatter(np.abs(choice_glms['s1_val_b'].loc[s1_sig & ~ofc_ix]),
                np.abs(choice_glms['s2_val_b'].loc[s1_sig & ~ofc_ix]), label='S1 only')
    plt.scatter(np.abs(choice_glms['s1_val_b'].loc[s2_sig & ~ofc_ix]),
                np.abs(choice_glms['s2_val_b'].loc[s2_sig & ~ofc_ix]), label='S2 only')
    plt.scatter(np.abs(choice_glms['s1_val_b'].loc[both_sig & ~ofc_ix]),
                np.abs(choice_glms['s2_val_b'].loc[both_sig & ~ofc_ix]), label='both')
    plt.xlim([0, .3])
    plt.ylim([0, .3])
    ...
    plt.scatter(np.abs(choice_glms['s1_val_int'].loc[s1_sig & ~ofc_ix]),
                np.abs(choice_glms['s2_val_int'].loc[s1_sig & ~ofc_ix]), label='S1 only')
    plt.scatter(np.abs(choice_glms['s1_val_int'].loc[s2_sig & ~ofc_ix]),
                np.abs(choice_glms['s2_val_int'].loc[s2_sig & ~ofc_ix]), label='S2 only')
    plt.scatter(np.abs(choice_glms['s1_val_int'].loc[both_sig & ~ofc_ix]),
                np.abs(choice_glms['s2_val_int'].loc[both_sig & ~ofc_ix]), label='both')
    plt.xlim([0, 1.4])
    plt.ylim([0, 1.4])
