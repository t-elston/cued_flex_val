...
for f = 1:numel(f_data)
    ...
    for u = 1:n_units
        ...
        cue_FRs(:, u) = zscore([nanmean(choice_data(u_ix & bhv.state == 1 & bhv.state_type == 1)) ;
            nanmean(choice_data(u_ix & bhv.state == 1 & bhv.state_type == -1)) ;
            nanmean(choice_data(u_ix & bhv.state == -1 & bhv.state_type == 1)) ;
            nanmean(choice_data(u_ix & bhv.state == -1 & bhv.state_type == -1))]);
        
        choice_FRs(:, u) = zscore([nanmean(choice_data(u_ix & bhv.state == 1 & bhv.chosenval == 1)) ;
            nanmean(choice_data(u_ix & bhv.state == 1 & bhv.chosenval == 2)) ;
            nanmean(choice_data(u_ix & bhv.state == 1 & bhv.chosenval == 3)) ;
            nanmean(choice_data(u_ix & bhv.state == 1 & bhv.chosenval == 4)) ;
            nanmean(choice_data(u_ix & bhv.state == -1 & bhv.chosenval == 1));
            nanmean(choice_data(u_ix & bhv.state == -1 & bhv.chosenval == 2)) ;
            nanmean(choice_data(u_ix & bhv.state == -1 & bhv.chosenval == 3)) ;
            nanmean(choice_data(u_ix & bhv.state == -1 & bhv.chosenval == 4))]);
    ...
    [cue_coeff, cue_score, ~, ~, cue_expvar, cue_mu] = pca(cue_FRs, 'NumComponents',4);
    [choice_coeff, choice_score, ~, ~, choice_expvar, choice_mu] = pca(choice_FRs, 'NumComponents',7);
    ...
    parfor b = 1:n_boots
        ...
        for u = 1:n_units
            ...
            b_cue_FRs(:, u) = zscore([nanmean(choice_data(c1_t1)) ;
                nanmean(choice_data(c1_t2)) ;
                nanmean(choice_data(c2_t1)) ;
                nanmean(choice_data(c2_t2))]);
        ...
        % project this bootstrap's PCs and aggregate
         [cue_PCs(:,:,b)] = ProjectIntoPCspace_v02(b_cue_FRs,cue_mu,cue_coeff,3);
         [choice_PCs(:,:,b)] = ProjectIntoPCspace_v02(b_choice_FRs,choice_mu,choice_coeff,7);
