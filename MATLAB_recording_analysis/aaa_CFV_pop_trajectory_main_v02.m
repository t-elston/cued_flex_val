...
for f = 1:numel(f_data)
    ...
    for u = 1:n_units
        ...
        n_choice_conds(:,u) = [ sum(u_ix & bhv.state == 1 & bhv.chosenval == 1) ; 
                                sum(u_ix & bhv.state == 1 & bhv.chosenval == 2) ;
                                sum(u_ix & bhv.state == 1 & bhv.chosenval == 3) ;
                                sum(u_ix & bhv.state == 1 & bhv.chosenval == 4) ;
                                sum(u_ix & bhv.state == -1 & bhv.chosenval == 1) ; 
                                sum(u_ix & bhv.state == -1 & bhv.chosenval == 2) ;
                                sum(u_ix & bhv.state == -1 & bhv.chosenval == 3) ;
                                sum(u_ix & bhv.state == -1 & bhv.chosenval == 4)] ;
        ...
        choice_FRs(:, u) = zscore([ ;
            nanmean(choice_data(u_ix & bhv.chosenval == 1)) ;
            nanmean(choice_data(u_ix & bhv.chosenval == 2)) ;
            nanmean(choice_data(u_ix & bhv.chosenval == 3)) ;
            nanmean(choice_data(u_ix & bhv.chosenval == 4)) ;
            nanmean(choice_data(u_ix & bhv.state == 1)) ;
            nanmean(choice_data(u_ix & bhv.state == -1)) ;
            ]);
    ...
    [cue_coeff, cue_score, ~, ~, cue_expvar, cue_mu] = pca(cue_FRs);
    [choice_coeff, choice_score, ~, ~, choice_expvar, choice_mu] = pca(choice_FRs);
    n_cue_PCs = 3;
    n_choice_PCs = 4;
