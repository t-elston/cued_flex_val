% sliding_window_value_decoder_v01.m
...
for s = 1:numel(subj_names)
    ...
    for a = 1:numel(brain_areas)
        ...
        % extract firing rates and timestamps
        FRs = data.zFRs;
        t_mids = data.ts+1;
        ...
        % intialize array for accuracies across bootstraps
        boot_acc=NaN(n_boots, n_times);
        boot_firing_rates = [];

        % loop over bootstraps
        pw = PoolWaitbar(numel(neuron_names), 'bootstrapping decoder...');
